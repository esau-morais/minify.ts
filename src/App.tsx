...
const App = (): JSX.Element => {
  ...
  const combineWords = async (secondWord: string): Promise<unknown> => {
    try {
      ...
      if (!result.data.choices?.[0].text) return new Error('Invalid response')
  ...
  return (
    <FormProvider {...methods}>
      <form
        ...
        onSubmit={formSubmit(handleSubmit) as any}
      >
        ...
        <div className='flex flex-col space-y-6 md:grid md:grid-cols-12 justify-center items-center w-full mb-4 py-16'>
          ...
          <input
            className={cx(
              ...
              errors.adjective ? 'border-red-500' : 'border-gray-600'
            )}
            placeholder='boring'
            {...register('adjective')}
            type='text'
          ...
          <div className='text-center  w-96 col-span-4'>
            <p
              className={'cursor-pointer text-center text-xl sm:text-4xl lg:text-5xl xl:text-6xl font-bold font-serif'}>
            </p>
          </div>
        </div>
